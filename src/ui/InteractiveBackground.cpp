...
void InteractiveBackground::initializeClouds() {
    ...
    for (int i = 0; i < numberOfClouds; ++i) {
        ...
        int startX = QRandomGenerator::global()->bounded(100, 200);
        int startY = QRandomGenerator::global()->bounded(100, 500);
        ...
        cloud.radius = QRandomGenerator::global()->bounded(30, 60);
        int alpha = QRandomGenerator::global()->bounded(100, 200);
        cloud.color = QColor(255, 255, 255, alpha);
...
void InteractiveBackground::paintEvent(QPaintEvent *event) {
    ...
    // Draw the solid background color (e.g., the light blue we used before)
    painter.fillRect(rect(), QColor(200, 230, 250)); // Or your specific leftWidget color
        ...
        if (isMouseOver) {
            ...
            if (distanceSquared < mouseDetectionRadius * mouseDetectionRadius) {
                ...
                if (distance > 0.1) { // Avoid division by zero
                    // Normalize the vector and scale by force (inverse distance would be better)
                    QPointF repulsionVector = (vectorFromMouse / distance) * mouseRepulsionForce;
